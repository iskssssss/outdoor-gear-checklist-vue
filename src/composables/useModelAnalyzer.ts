...
interface ProcessedItem extends AnalyzedItem {
  id: string | number;
}

export function useModelAnalyzer(modelConfigStore: ReturnType<typeof useModelConfigStore>, equipmentStore: ReturnType<typeof useEquipmentStore>) {
  /**
   * 检测商品名称中是否含有套装信息
   * 支持格式：
   *   - “5支装”、“10包”、“3件”、“二支装”、“一套”、“家庭装”、“组合装”等
   */
  function detectBundleInfo(name: string): BundleInfo | null {
    // 匹配阿拉伯数字或中文数字 + 常见单位
    const match = name.match(/(\d+|[一二三四五六七八九十]+)\s*(支|包|件|个|对|瓶|片|条|套)/u);
    if (!match) return null;

    let num: string | number = match[1];
    // 中文数字转阿拉伯数字
    if (isNaN(Number(num))) {
   ...
   */
  function adjustBundlePrice(item: AnalyzedItem): AnalyzedItem {
    const bundle = detectBundleInfo(item.name);
    if (!bundle) return item;
    const adjusted: AnalyzedItem = { ...item };
   ...
   * 主函数：分析商品并分类
   */
  async function analyzeWithModel(items: RawItem[]): Promise<ProcessedItem[]> {
    if (!modelConfigStore.settings.apiKey && !modelConfigStore.settings.apiUrl.includes('localhost')) {
      throw new Error('未配置模型 API');
    }

    // 先预识别套装信息
    const enrichedItems: EnrichedItem[] = items.map(i => ({
      ...i,
      bundleInfo: detectBundleInfo(i.name)
    }));

    // 获取现有分类
    const existingCategories: string = equipmentStore.categories.map(c => c.name).join('、');
    ...
    try {
      ...
      let parsed: AnalyzedItem[] = JSON.parse(jsonStr);
      ...
      // 进行后处理：调整套装价格
      const adjustedResults: AnalyzedItem[] = parsed.map(adjustBundlePrice);

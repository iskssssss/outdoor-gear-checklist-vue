    ...
    <div class="edit-segment-modal">
      <form @submit.prevent="handleSubmit">
        <div class="form-section">
          ...
          <div class="form-row">
            <BaseFormField label="出发地" required>
              <BaseInput
            ...
            <BaseFormField label="目的地" required>
              <BaseInput
          ...
          <div class="form-row">
            <BaseFormField label="出发时间" required>
              <BaseInput
          ...
          <div class="form-row">
            <BaseFormField label="基础费用（元）" required>
              <BaseInput
...
const emit = defineEmits<{
  close: []
  save: [updates: Partial<TransportSegment>]
}>()
...
const formData = reactive({
  transportType: props.segment.transportType,
  departure: {
    name: props.segment.departure.name,
    address: props.segment.departure.address || '',
    coordinates: props.segment.departure.coordinates
  },
  arrival: {
    name: props.segment.arrival.name,
    address: props.segment.arrival.address || '',
    coordinates: props.segment.arrival.coordinates
  },
  departureTime: props.segment.departureTime.toISOString().slice(0, 16),
  arrivalTime: props.segment.arrivalTime.toISOString().slice(0, 16),
  duration: props.segment.duration,
  distance: props.segment.distance,
  cost: {
    baseCost: props.segment.cost.baseCost,
    totalCost: props.segment.cost.totalCost,
    currency: props.segment.cost.currency,
    otherCosts: [...(props.segment.cost.otherCosts || [])]
  },
  notes: props.segment.notes || ''
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 构建更新数据
    const updates: Partial<TransportSegment> = {
